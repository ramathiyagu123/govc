...
function create_iso() {
  ...
  echo "binaries/govc datastore.upload -ds=${VSPHERE_IMAGE_DATASTORE} binaries/${OPENSHIFT_CLUSTERNAME}-${TYPE}-${COUNT}.iso ${VSPHERE_IMAGE_DATASTORE_PATH}/${OPENSHIFT_CLUSTERNAME}-${TYPE}-${COUNT}.iso  > /dev/null"
...
EOF
  cp ${OPENSHIFT_CLUSTERNAME}/install-config.yaml /opt/backup_files/
